...
def _door_arc_evidence_details(
    ...
    # HoughCircles scales poorly with large ROIs. Door swing evidence does not
    # require source-resolution pixels, so cap the working ROI and convert all
    # accepted geometry back to source coordinates before returning.
    roi_h,roi_w=roi.shape[:2]
    max_dim=max(roi_h,roi_w)
    work_scale=min(1.0,360.0/max(1.0,float(max_dim)))
    if work_scale<.999:
        work=cv2.resize(
            roi,
            (
...
def detect_doors(
    ...
    for index,left in enumerate(walls):
        ...
        for right in walls[index+1:]:
            ...
            # HoughCircles is substantially more expensive than leaf-line
            # evidence on large construction sheets. Only evaluate swing arcs
            # when leaf evidence alone is insufficient to accept the door.
