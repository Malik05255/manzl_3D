...
def extract_secondary_onnx_detections(image:np.ndarray)->list[dict]:
    ...
    raw_allowed=os.getenv("SYMBOL_SECONDARY_ALLOWED_CLASSES","").strip()
    if not raw_allowed:
        return detections
    if raw_allowed.startswith("["):
        try:
            ...
            parsed=[]
        allowed={
            " ".join(str(item).strip().lower().replace("_"," ").replace("-"," ").split())
            for item in parsed
            if str(item).strip()
        }
    else:
        allowed={
            " ".join(item.strip().lower().replace("_"," ").replace("-"," ").split())
