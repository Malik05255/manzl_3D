...
function Editor({initialProject,onHome}:{initialProject:ProjectView;onHome:()=>void}){
  ...
  const ask=async()=>{if(!command.trim())return;setThinking(true);setNotice(null);setPreview(null);try{await syncPlanForEngineer();const r=await askEngineer(project.id,command.trim(),selectedRoom);setProposals(r.proposals);if(r.needsClarification)setNotice(r.needsClarification);}catch(e){setNotice(e instanceof Error?e.message:"تعذر تحليل الطلب");}finally{setThinking(false);}};
  ...
  const apply=async()=>{if(!preview)return;setSaving(true);++draftGeneration.current;try{await draftChain.current.catch(()=>undefined);const u=await applyProposal(project.id,{command,proposal:preview,targetRoomId:selectedRoom});if(u.plan){setPlan(u.plan);setSavedPlan(u.plan);setUndoStack([]);setRedoStack([]);}setProject(u);setRecoveredDraft(false);setPreview(null);setProposals([]);setCommand("");setSelectedRoom(null);setSelectedOpening(null);setExactName("");setExactWidth("");setExactHeight("");setValidationReport(null);setNotice("تم اعتماد التعديل وحفظ نسخة جديدة.");}catch(e){const report=validationFromApiError(e);if(report)setValidationReport(report);setNotice(e instanceof Error?e.message:"تعذر تطبيق التعديل");}finally{setSaving(false);}};
  ...
  return <main className="editor-page">
    ...
    <div className="editor-workspace">
      ...
      <aside className="ai-panel"><div className="ai-title"><div className="ai-avatar"><BrainCircuit size={22}/></div><div><strong>H Engineer</strong><span>يفهم الأثر قبل التنفيذ</span></div></div>
        ...
        <div className="prompt-box">{selectedRoom&&<div className="prompt-context">الغرفة المحددة · {plan.rooms.find(room=>room.id===selectedRoom)?.name}</div>}<textarea value={command} onChange={e=>setCommand(e.target.value)} placeholder={selectedRoom?"مثال: اجعلها 5×5 أو زد عرضها متر":"مثال: عدّل غرفة النوم إلى 5×5"}/><button className="primary" disabled={thinking||!command.trim()} onClick={ask}>{thinking?<LoaderCircle className="spin" size={18}/>:<Sparkles size={18}/>} تحليل الطلب</button></div>
