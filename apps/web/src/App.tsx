...
import type { DoorSubtype,EditProposal,ElementProvenance,FloorPlanModel,Opening,Point,ProjectView,RevisionView,ValidationReport,WallRole } from "@manzil/contracts";
...
import { addOpeningToWall,changeDoorSubtype,changeOpeningKind,findOpening,openingMetrics,positionOpening,removeOpening,resizeOpening } from "./openingGeometry";
...
function Editor({initialProject,onHome}:{initialProject:ProjectView;onHome:()=>void}){
  ...
  const addManualWall=(a:Point,b:Point)=>{
    const next=createManualWall(plan,a,b);
    ...
    setNotice("تمت إضافة جدار يدوي. سيعيد الخادم ربط حدود الغرف عند الحفظ.");
  };
  const deleteSelectedWall=()=>{
    if(!selectedWall)return;
    const reason=wallRemovalReason(plan,selectedWall);
    if(reason){setNotice(reason);return;}
    const next=removeUnboundWall(plan,selectedWall);
    if(!next){setNotice("تعذر حذف الجدار بأمان.");return;}
    applyLocalPlan(next);
    setSelectedWall(null);setWallThicknessCm("");
    setNotice("تم حذف الجدار غير المرتبط. يمكنك التراجع قبل الحفظ.");
  };
  const splitSelectedWallMidpoint=()=>{
    if(!selectedWall)return;
    const wall=plan.walls.find(item=>item.id===selectedWall);
    if(!wall)return;
    const next=splitWallAtPoint(plan,selectedWall,{x:(wall.a.x+wall.b.x)/2,y:(wall.a.y+wall.b.y)/2});
    if(!next){setNotice("تعذر تقسيم الجدار. تأكد أنه غير محمي وطوله كافٍ.");return;}
    const before=new Set(plan.walls.map(item=>item.id));
    const created=next.walls.filter(item=>!before.has(item.id));
    applyLocalPlan(next);
    const first=created[0];
    if(first){
      setSelectedWall(first.id);
      setWallThicknessCm(next.metersPerPixel?(first.thicknessPx*next.metersPerPixel*100).toFixed(1):"");
    }
    setNotice("تم تقسيم الجدار مع تحديث مراجع الغرف والفتحات والأبعاد.");
  };
  const joinSelectedWall=()=>{
    if(!selectedWall)return;
    const wall=plan.walls.find(item=>item.id===selectedWall);
    if(!wall)return;
    const endpointDistance=(candidate:typeof wall)=>Math.min(
      Math.hypot(candidate.a.x-wall.a.x,candidate.a.y-wall.a.y),
      Math.hypot(candidate.a.x-wall.b.x,candidate.a.y-wall.b.y),
      Math.hypot(candidate.b.x-wall.a.x,candidate.b.y-wall.a.y),
      Math.hypot(candidate.b.x-wall.b.x,candidate.b.y-wall.b.y),
    );
    const candidates=plan.walls.filter(item=>item.id!==selectedWall).sort((a,b)=>endpointDistance(a)-endpointDistance(b));
    for(const candidate of candidates){
      const next=joinCollinearWalls(plan,selectedWall,candidate.id);
      if(!next)continue;
      applyLocalPlan(next);
      const joined=next.walls.find(item=>item.id===selectedWall);
      setWallThicknessCm(joined&&next.metersPerPixel?(joined.thicknessPx*next.metersPerPixel*100).toFixed(1):"");
      setNotice("تم دمج الجدار مع أقرب جزء متصل وعلى نفس المحور، مع تحديث المراجع.");
      return;
    }
    setNotice("لا يوجد جدار متصل ومتوافق هندسيًا يمكن دمجه مع الجدار المحدد.");
  };
  ...
  const resizeSelectedWallEndpoint=(endpoint:"a"|"b",mode:"trim"|"extend")=>{
    if(!selectedWall||!plan.metersPerPixel)return;
    const cm=Number(wallMoveCm.replace(",","."));
    if(!Number.isFinite(cm)||cm<=0||cm>1000){setNotice("أدخل مسافة بين 0 و1000 سم.");return;}
    const distancePx=(cm/100)/plan.metersPerPixel;
    const next=mode==="trim"
      ?trimWallEndpoint(plan,selectedWall,endpoint,distancePx)
      :extendWallEndpoint(plan,selectedWall,endpoint,distancePx);
    if(!next){
      setNotice(mode==="trim"
        ?"تعذر قص نهاية الجدار بأمان؛ قد يكون محميًا أو حدًا لغرفة أو يحمل فتحة قرب النهاية."
        :"تعذر تمديد نهاية الجدار؛ قد يكون محميًا أو حدًا لغرفة أو سيتجاوز حدود المخطط.");
      return;
    }
    applyLocalPlan(next);
  ...
  const updateDoorSubtype=(doorSubtype:DoorSubtype)=>{
    if(!selectedOpening)return;
    if(openingHostProtected(plan,selectedOpening)){setNotice("الفتحة على جدار محمي. فك حماية الجدار أولًا.");return;}
    const next=changeDoorSubtype(plan,selectedOpening,doorSubtype);
    if(!next)return;
    applyLocalPlan(next);
    setNotice("تم تحديث نوع الباب وتسجيل التصحيح اليدوي.");
  };
  const deleteOpening=()=>{
    if(!selectedOpening)return;
    if(openingHostProtected(plan,selectedOpening)){setNotice("الفتحة على جدار محمي. فك حماية الجدار أولًا.");return;}
  ...
  const rebuildRoomsPreview=async()=>{
    if(plan.walls.length<4){setNotice("صحح أو ارسم الحدود الرئيسية أولًا؛ لا توجد جدران كافية لإعادة بناء الغرف.");return;}
    const canonical="إعادة بناء الغرف من الجدران";
    setThinking(true);setNotice(null);setPreview(null);setProposals([]);
    try{
      await syncPlanForEngineer();
      const result=await askEngineer(project.id,canonical,project.revision,{});
      setCommand(canonical);setProposals(result.proposals);
      if(result.needsClarification)setNotice(result.needsClarification);
    }catch(e){setNotice(e instanceof Error?e.message:"تعذر إعادة بناء الغرف من الجدران الحالية.");}
    finally{setThinking(false);}
  };
  ...
  useEffect(()=>{
    const handle=(event:KeyboardEvent)=>{
      ...
      if(event.key==="Escape"){
        setLayerOpen(false);setFloorSettingsOpen(false);setPreview(null);setProposals([]);setSelectedWall(null);setSelectedRoom(null);setSelectedOpening(null);setSelectedDimension(null);
        return;
      }
      if(typing)return;
  ...
  const loadFloorProject=(fresh:ProjectView,message:string)=>{
    ...
    setSelectedWall(null);setWallThicknessCm("");setSelectedRoom(null);setSelectedOpening(null);setSelectedDimension(null);
  ...
  const focusValidationFinding=(item:ValidationReport["findings"][number])=>{
    ...
    if(item.dimensionIds?.[0]){selectDimension(item.dimensionIds[0]);return;}
  ...
  const apply=async()=>{if(!preview)return;setSaving(true);++draftGeneration.current;try{await draftChain.current.catch(()=>undefined);const u=await applyProposal(project.id,{command,proposal:preview,expectedRevision:project.revision,targetRoomId:selectedRoom,targetWallId:selectedWall,targetOpeningId:selectedOpening});if(u.plan){setPlan(u.plan);setSavedPlan(u.plan);setUndoStack([]);setRedoStack([]);}setProject(u);setRecoveredDraft(false);setPreview(null);setProposals([]);setCommand("");setSelectedRoom(null);setSelectedOpening(null);setSelectedDimension(null);setExactName("");setExactWidth("");setExactHeight("");setValidationReport(null);setNotice("تم اعتماد التعديل وحفظ نسخة جديدة.");}catch(e){const report=validationFromApiError(e);if(report)setValidationReport(report);setNotice(e instanceof Error?e.message:"تعذر تطبيق التعديل");}finally{setSaving(false);}};
  ...
  ["uncertainty","إبراز القراءة غير المؤكدة"],
      ...
      <aside className="ai-panel"><div className="ai-title"><div className="ai-avatar"><BrainCircuit size={22}/></div><div><strong>H Engineer</strong><span>يفهم الأثر قبل التنفيذ</span></div></div>
        ...
        <div className="prompt-box">{selectedContextLabel&&<div className="prompt-context">{selectedContextLabel}</div>}<textarea disabled={Boolean(selectedDimension)} value={command} onChange={e=>setCommand(e.target.value)} placeholder={selectedDimension?"صحح البعد أو استخدمه للمعايرة من البطاقة أعلاه":selectedOpening?"مثال: اجعل عرضه 90 سم أو حركه يمين 30 سم":selectedWall?"مثال: حركه يمين 30 سم أو اجعل سماكته 20 سم":selectedRoom?"مثال: اجعلها 5×5 أو زد عرضها متر":"مثال: عدّل غرفة النوم إلى 5×5"}/><button className="primary" disabled={thinking||!command.trim()||Boolean(selectedDimension)} onClick={ask}>{thinking?<LoaderCircle className="spin" size={18}/>:<Sparkles size={18}/>} تحليل الطلب</button></div>
