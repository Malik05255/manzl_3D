...
function Editor({initialProject,onHome}:{initialProject:ProjectView;onHome:()=>void}){
  ...
  const reviewItems=useMemo(()=>{
    const items:Array<{key:string;kind:"room"|"wall"|"opening";id:string;label:string;confidence:number}>=[];
    for(const room of plan.rooms)if(room.confidence<.78)items.push({key:`room:${room.id}`,kind:"room",id:room.id,label:room.name||"غرفة غير مسماة",confidence:room.confidence});
    for(const wall of plan.walls)if(wall.confidence<.72)items.push({key:`wall:${wall.id}`,kind:"wall",id:wall.id,label:"جدار يحتاج تأكيد",confidence:wall.confidence});
    for(const opening of [...plan.doors,...plan.windows])if(opening.confidence<.84)items.push({key:`opening:${opening.id}`,kind:"opening",id:opening.id,label:opening.kind==="door"?"باب يحتاج تأكيد":"نافذة تحتاج تأكيد",confidence:opening.confidence});
  ...
  const confirmSelectedReading=()=>{
    ...
    if(!item)return;
    if(item.kind==="room"){
      applyLocalPlan({...plan,rooms:plan.rooms.map(room=>room.id===item.id?{...room,confidence:1}:room)});
    }else if(item.kind==="wall"){
      applyLocalPlan({...plan,walls:plan.walls.map(wall=>wall.id===item.id?{...wall,confidence:1}:wall)});
    }else{
      applyLocalPlan({
        ...plan,
        doors:plan.doors.map(opening=>opening.id===item.id?{...opening,confidence:1}:opening),
        windows:plan.windows.map(opening=>opening.id===item.id?{...opening,confidence:1}:opening),
      });
    }
  ...
  const apply=async()=>{if(!preview)return;setSaving(true);++draftGeneration.current;try{await draftChain.current.catch(()=>undefined);const u=await applyProposal(project.id,{command,proposal:preview,targetRoomId:selectedRoom,targetWallId:selectedWall,targetOpeningId:selectedOpening});if(u.plan){setPlan(u.plan);setSavedPlan(u.plan);setUndoStack([]);setRedoStack([]);}setProject(u);setRecoveredDraft(false);setPreview(null);setProposals([]);setCommand("");setSelectedRoom(null);setSelectedOpening(null);setExactName("");setExactWidth("");setExactHeight("");setValidationReport(null);setNotice("تم اعتماد التعديل وحفظ نسخة جديدة.");}catch(e){const report=validationFromApiError(e);if(report)setValidationReport(report);setNotice(e instanceof Error?e.message:"تعذر تطبيق التعديل");}finally{setSaving(false);}};
