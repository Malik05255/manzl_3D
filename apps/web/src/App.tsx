...
function Editor({initialProject,onHome}:{initialProject:ProjectView;onHome:()=>void}){
  ...
  const reviewItems=useMemo(()=>{
    ...
    for(const room of plan.rooms)if(!room.reviewed&&room.confidence<.78)items.push({key:`room:${room.id}`,kind:"room",id:room.id,label:room.name||"غرفة غير مسماة",confidence:room.confidence});
    for(const wall of plan.walls)if(!wall.reviewed&&wall.confidence<.72)items.push({key:`wall:${wall.id}`,kind:"wall",id:wall.id,label:"جدار يحتاج تأكيد",confidence:wall.confidence});
    for(const opening of [...plan.doors,...plan.windows])if(!opening.reviewed&&opening.confidence<.84)items.push({key:`opening:${opening.id}`,kind:"opening",id:opening.id,label:opening.kind==="door"?"باب يحتاج تأكيد":"نافذة تحتاج تأكيد",confidence:opening.confidence});
